...
public class FastCorrelationBasedFilterTest {
    ...
    public void testBasic() {
        ...
        Instance in = new SimpleInstance(tmp,1,true,1);
    ...
    public void testRandom(){
        ...
        for(int i=0;i<10;i++){
            ...
            Instance in = new SimpleInstance(tmp,1,true,7);
            data.addInstance(in);
        }
        for(int i=0;i<10;i++){
            double[] tmp = { 4, 1, 3, 3, 3, 3, 6, 4, 5, 6, 7, 8, 9, 7, 5, 3 };
            Instance in = new SimpleInstance(tmp,1,true,6);
            data.addInstance(in);
        }
        for(int i=0;i<10;i++){
        ...
        for(int i=0;i<10;i++){
            ...
            Instance in = new SimpleInstance(tmp,1,true,0);
            data.addInstance(in);
        }
        
        for(int i=0;i<10;i++){
            double[] tmp = { 4, 1, 2, 2, 3, 3, 7, 5, 5, 6, 7, 5, 4, 7, 5, 5 };
            Instance in = new SimpleInstance(tmp,1,true,2);
            data.addInstance(in);
        }
        FastCorrelationBasedFilter filter=new FastCorrelationBasedFilter();
        filter.filterDataset(data);
