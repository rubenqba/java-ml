...
class Cache {
    ...
    Cache(int l_, long size_) {
        ...
        size = Math.max(size, 2 * (long) l); // cache must be large enough
                                                // for two columns
        lru_head = new head_t();
...
//            System.out.print("Total nSV = " + nnz + "\n");
    ...
    public static void svm_predict_values(svm_model model, svm_node[] x, double[] dec_values) {
        if (model.param.svm_type == svm_parameter.ONE_CLASS || model.param.svm_type == svm_parameter.EPSILON_SVR
            ...
            // System.out.println("KValue = "+Arrays.toString(kvalue));
            int[] start = new int[nr_class];
            start[0] = 0;
            for (i = 1; i < nr_class; i++)
            ...
            for (i = 0; i < nr_class; i++)
                for (int j = i + 1; j < nr_class; j++) {
                    ...
                    // System.out.println("COEF1 - "+p+ "
                    // :"+Arrays.toString(coef1));
                    // System.out.println("COEF2:"+Arrays.toString(coef2));
                    sum -= model.rho[p];
                    dec_values[p] = sum;
                    p++;
                }
