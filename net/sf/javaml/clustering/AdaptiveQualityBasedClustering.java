...
public class AdaptiveQualityBasedClustering implements Clusterer {
	...
	private class AQBCem {
		...
		// instances (checked)
		private double var(Vector<Instance> cluster, double dimD,
				int instanceLength) {
			double var;
			double sum = 0;
		...
		private double varOp(Vector<Instance> cluster, Vector<Double> pcr,
		...
		private Vector<Double> prc(double var, Vector<Double> clusterDist,
		...
		private Vector<Double> prb(double var, Vector<Double> clusterDist,
		...
		private double sD(double dimD) {
			double sD = Math.pow(2 * Math.PI, (dimD / 2))
					/ gammaF.gamma(dimD / 2);
			return sD;
		}
		 ...
		 */
		// main algorithm
		private double em(int maxIter, Vector<Instance> dataset,
				Vector<Instance> cluster, Instance ck, double rk_prelim,
				double dimension, int instanceLength, Vector<Double> varianceEst) {
			dimD = dimension - 2;
	...
	/**
	 * XXX add doc
	 * @param int maximum iterations of main algorithm
	 * @param int maximum iterations to find a cluster
	 * @param int maximum iterations of EM algorithm
	 * @param double significanceLevel
	 * @param double percentage of data left when main algorithm has to stop
	 * 
	 */

	public AdaptiveQualityBasedClustering(int maxIterMain, int maxIter,
			int maxIterEM, double significanceLevel, double percent) {
	...
	public Dataset[] executeClustering(Dataset data) {
		...
		while (iterator < maxIterMain && endSign != 0) {
			...
			double pc = em.em(maxIterEM,all, cluster, ck, rk_prelim, dimension,
					instanceLength, varianceEst);
			double pb = 1 - pc;
