...
public class XMeans implements Clusterer {
    ...
    protected double bic = Double.MIN_VALUE;
    ...
    protected DistanceMeasure dm = new EuclideanDistance();

    /** cluster centers */
    protected Instance[] m_ClusterCenters;
    ...
    // protected File m_DebugVectorsFile = new
    ...
    //    
    // /** file name of the output file for the cluster centers */
    // protected File m_OutputCenterFile = new
    // File(System.getProperty("user.dir"));
    //  
    ...
    public Dataset[] executeClustering(Dataset data) {
        ...
        PFD(D_FOLLOWSPLIT, "\n*** Starting centers ");
        for (int k = 0; k < m_ClusterCenters.length; k++) {
            PFD(D_FOLLOWSPLIT, "Center " + k + ": " + m_ClusterCenters[k]);
        }
        ...
        while (!finished && !stopIteration(m_IterationCount, m_MaxIterations)) {
            ...
            ; i++) {
                ...
                children = splitCenter(rg, currCenter, variance, m_Model);
     ...
     */
    protected boolean recomputeCenters(Instance[] centers, int[][] instOfCent, Dataset model) {
            ...
            for (int j = 0; j < model.numAttributes(); j++) {
                ...
                for (int k = 0; k < instOfCent[i].length; k++)
                    ...
                    if (converged && m_ClusterCenters[i].getValue(j) != val)
                        converged = false;
                if (!converged)
                    m_ClusterCenters.getInstance(i).setValue(j, val);
            }
        }
        return converged;
    }
     ...
     *            data model information
     */
    protected void recomputeCentersFast(Instance[] centers, int[][] instOfCentIndexes, Dataset model) {
        for (int i = 0; i < centers.length        ; i++) {
            double val;
            for (int j = 0; j < model.numAttributes(); j++) {
                val = meanOrMode(data, instOfCentIndexes[i], j);
                centers.getInstance(i).setValue(j, val);
            }
        }
    }
     ...
     * @return mean value
     */
    protected double meanOrMode(Dataset instances, int[] instList, int attIndex) {
        double result, found;
        int[] counts;
     ...
     */
    protected boolean assignToCenters(KDTree tree, Instance[] centers, int[][] instOfCent, int[] allInstList,
            int[] assignments, int iterationCount) throws Exception {
     ...
     */
    protected boolean assignToCenters(KDTree kdtree, Instance[] centers, int[][] instOfCent, int[] assignments,
     ...
     */
    protected boolean assignToCenters(Instance[] centers, int[][] instOfCent, int[] allInstList, int[] assignments)
            throws Exception {
     ...
     */
    protected Instance[] splitCenter(Random random, Instance center, double variance, Dataset model) throws Exception {
        m_NumSplits++;
        AlgVector r = null;
        Instance[] children = new Instances(model, 2);
    ...
    protected double calculateBIC(int[][] instOfCent, Instances centers, double[] mle) {
        ...
        for (int i = 0; i < centers.numInstances(); i++) {
            loglike += logLikelihoodEstimate(instOfCent[i].length, centers.instance(i), mle[i],
                    centers.numInstances() * 2);
     ...
     */
    protected double[] distortion(int[][] instOfCent, Instances centers) throws Exception {
        double[] distortion = new double[centers.numInstances()];
        for (int i = 0; i < centers.numInstances(); i++) {
            distortion[i] = 0.0;
            for (int j = 0; j < instOfCent[i].length; j++) {
                distortion[i] += dm.distance(m_Instances.instance(instOfCent[i][j]), centers.instance(i));
            }
        }
        /*
     ...
     */
    protected int clusterProcessedInstance(Instance instance, Instances centers) throws Exception {

        double minDist = Integer.MAX_VALUE;
        int bestCluster = 0;
        for (int i = 0; i < centers.numInstances(); i++) {
            double dist = dm.distance(instance, centers.instance(i));
    ...
    protected int clusterProcessedInstance(Instance instance) throws Exception {
        ...
        for (int i = 0; i < m_NumClusters; i++) {
            double dist = dm.distance(instance, m_ClusterCenters.instance(i));
            if (dist < minDist) {
                minDist = dist;
                bestCluster = i;
            }
    ...
    protected String getDistanceFSpec() {
        ...
        if (d instanceof OptionHandler) {
            return d.getClass().getName() + " " + Utils.joinOptions(((OptionHandler) d).getOptions());
        }
    ...
    public String toString() {
        ...
        temp.append("BIC-Value : " + Utils.doubleToString(bic, 6) + "\n");
