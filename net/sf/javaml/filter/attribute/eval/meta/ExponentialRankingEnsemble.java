...
public class ExponentialRankingEnsemble implements IAttributeRanking {

    private IAttributeRanking[] aes;

    public ExponentialRankingEnsemble(IAttributeRanking[] aes) {
        this.aes = aes;
    }
    ...
    public void build(Dataset data) {
        ...
        for (IAttributeRanking ae : aes) {
            ...
            for (int i = 0; i < numAtt; i++)
                sum[i] += Math.exp(ae.getRank(i));// * ae.getRank(i) * ae.getRank(i);
        }
        toRank(sum);
