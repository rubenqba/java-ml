...
public class MorletWavelet extends AbstractRealWaveletTransform {
    ...
    public MorletWavelet(int nvoice, int oct, int scale) {
    ...
    Complex[] transform(double[] omega, double omega0) {
        // OK window = exp(-(omega - omega0).^2 ./2) - exp(-(omega.^2 +
        // omega0.^2)/2);
        Complex[] window = new Complex[omega.length];
        for (int i = 0; i < window.length; i++) {
            window[i] = new Complex(Math.exp(-Math.pow((omega[i] - omega0), 2) / 2)
                    - Math.exp(-(Math.pow(omega[i], 2) + Math.pow(omega0, 2)) / 2), 0);
