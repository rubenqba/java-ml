...
public class GaussWavelet extends AbstractRealWaveletTransform {

    public GaussWavelet(int nvoice, int oct, int scale) {
    ...
    Complex[] transform(double[] omega, double omega0) {
        ...
        for (int i = 0; i < window.length; i++) {
            window[i] = new Complex(Math.exp(-(omega[i] * omega[i]) / 2), 0);
        }
        return window;
    }
