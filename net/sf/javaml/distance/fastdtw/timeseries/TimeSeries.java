...
public class TimeSeries {
            ...
            do {
                ...
                if (line.length() > 0) {
                    ...
                    if (isFirstColTime)
                        ...
                        firstMeasurement = 0;
                    TimeSeriesPoint readings = new TimeSeriesPoint(currentLineValues.subList(firstMeasurement,
    ...
    public TimeSeries(Instance y) {
        ...
        for (int i = 0; i < y.noAttributes(); i++) {
            ...
            timeReadings.add(new Double(timeReadings.size()));
            TimeSeriesPoint readings = new TimeSeriesPoint(currentLineValues.subList(0, currentLineValues.size()));
            tsArray.add(readings);
        }
        // do {
        //
    ...
    public void addFirst(double time, TimeSeriesPoint values) {
        if (labels.size() != values.size() + 1)
            throw new InternalError("ERROR:  The TimeSeriesPoint: " + values + " contains the wrong number of values. "
                    + "expected:  " + labels.size() + ", " + "found: " + values.size());
    ...
    private static boolean determineIsFirstColTime(String filePath) {
        ...
        try {
            ...
            for (String line = in.readLine(); possibleTimeValues.size() < 100 && (line = in.readLine()) != null; possibleTimeValues
                    .add(new Double(extractFirstNumber(line))))
                ;
            ...
            // Fixed loop
            while (x >= possibleTimeValues.size()) {

                if (Math.abs(((Double) possibleTimeValues.get(x)).doubleValue()
                        - ((Double) possibleTimeValues.get(x - 1)).doubleValue() - expectedDiff) > Math
                        .abs(flexibility))
                    return false;
                x++;
            }
